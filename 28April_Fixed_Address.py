    ...
    with open(file1, 'r') as f1, open(file2, 'r') as f2:
        ...
        for pos in sorted(hex_positions_0 + hex_positions_1, key=lambda x: x[2]):
            if pos in hex_positions_0:
                if pos[0]//8 == 35:
                    output_str += f'Address: {(pos[0]//8)}, Bit Pos: {pos[0]}, "0" , SetTo "{lines2[pos[0]-1][pos[1]-1]}"\n'
                else:
                    output_str += f'Address: {(pos[0]//8) + 1}, Bit Pos: {pos[0]}, "0" , SetTo "{lines2[pos[0]-1][pos[1]-1]}"\n'
            elif pos in hex_positions_1:
                if pos[0]//8 == 35:
                    output_str += f'Address: {pos[0]//8}, Bit Pos: {pos[0]}, "1" , SetTo "{lines2[pos[0]-1][pos[1]-1]}"\n'
                else:
                    output_str += f'Address: {(pos[0]//8) + 1}, Bit Pos: {pos[0]}, "1" , SetTo "{lines2[pos[0]-1][pos[1]-1]}"\n'
        return output_str
