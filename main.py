            ...
            for i in range(len(section_results)):
                for j in range(i+1, len(section_results)):
                    ...
                    if section == "temp" and not first_comparison_processed:
                        min_input1 = input1_values
                        first_comparison_processed = True
                        hex_pos = hex_pos

                    # Update max_input2 if input2_values is greater
                    max_input2 = max(max_input2, input2_values)
        if section == "temp":
            # Print the maximum input2 value in the output file
            f.write('"MaxInput2": ' + str(max_input2) + '\n')
